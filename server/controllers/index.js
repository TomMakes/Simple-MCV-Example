...
const hostPage1 = (req, res) => {
  ...
  const callback = (err, docs) => {
    ...
    docs.forEach(function (element) {
      if (this.beds) { catAr.push(element); }
    });
...
const hostPage4 = (req, res) => {
  const callback = (err, docs) => {
    ...
    docs.forEach((element) => {
      if (!req.body.beds) { dogAr.push(element); }
    });
...
// This will do both the Looking up of a dog name and updating its age
const searchUpdateDogName = (req, res) => {
  if (!req.query.name) {
    return res.json({ error: 'Name is required to perform a search' });
  }
  ...
  return Dog.findByName(req.query.name, (err, doc) => {
    ...
    // if a match, send the match back, but update age first
    lastDogAdded = doc;
    lastDogAdded.age++;
